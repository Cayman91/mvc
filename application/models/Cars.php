...
class Cars extends Model
{
    ...
    {
        $sth = $this->db->prepare('SELECT samochody.*, marki.* FROM samochody LEFT JOIN marki ON samochody.marki_id = marki.marki_id ORDER BY samochody_id DESC');
        $sth->execute();
        return $sth->fetchAll();
    }

    public function getSamochodyLimit($from, $limit)
    {
        $sth = $this->db->prepare('SELECT samochody.*, marki.* FROM samochody LEFT JOIN marki ON samochody.marki_id = marki.marki_id ORDER BY samochody_id DESC LIMIT '.$from.', '.$limit.'');
        $sth->execute();
        return $sth->fetchAll();
    }

    public function deleteSamochody($idAuta)
    {
